...
F = lambda x: (10*mt.exp(-0.5*x)*mt.cos(2*x) -4)

F_deriv = lambda x: mt.exp(-0.5*x)*(-20*mt.sin(2*x)-5*mt.cos(2*x))

F_dderiv = lambda x: mt.exp(-0.5*x)*(20*mt.sin(2*x)-37.5*mt.cos(2*x))
...
def newtonMethod(x0,a,b,x_der_min,x_dder_max,eps):
    min_fd=F_deriv(x_der_min)
    max_fdd=F_dderiv(x_dder_max)
    m=(1/2)*(min_fd/max_fdd)
